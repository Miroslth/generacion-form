...
function normalizeLanguageName(languageName) {
  return languageName.startsWith('@/') ? `js/${languageName.slice(2)}` : languageName;
}
...
function doesRuleSupportLanguage(ruleLangs, configLanguageName, validPluginNames) {
  ...
  const { objectName: configLangPart } = splitPluginIdentifier(configLanguageName);
  ...
  for (const langEntry of ruleLangs) {
    ...
    if (ruleLangPart === '*') {
      ...
      if (validPluginNames.includes(rulePluginPart) && ruleLangPart === configLangPart) {
        return true;
      }
    }
  }
